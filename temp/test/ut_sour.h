...
TEST(SOUR,test_sour_is_default_should_throw_exception){
    Sour sour;
    ASSERT_THROW(sour.get_name(),std::string);
    ASSERT_THROW(sour.get_price(),std::string);
    ASSERT_THROW(sour.get_alcohol_content(),std::string);
}
...
TEST(SOUR,test_sour_contains_price_is_negative_should_throw_exception){
    ASSERT_THROW(Sour("abcd",-1,0.1),std::string);

}

TEST(SOUR,test_sour_contains_content_is_negative_should_throw_exception){
    ASSERT_THROW(Sour("abcd",5,-0.1),std::string);
}
