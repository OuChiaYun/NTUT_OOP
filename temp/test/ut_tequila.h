...
TEST(TEQUILA,test_sour_is_default_should_throw_exception){
    Tequila tequila;
    ASSERT_THROW(tequila.get_name(),std::string);
    ASSERT_THROW(tequila.get_price(),std::string);
    ASSERT_THROW(tequila.get_alcohol_content(),std::string);
}
...
TEST(TEQUILA,test_Tequila_delipute_alcohol_reduce_5_persent){
    ...
    Tequila tequila("item1",2,0.3);
    tequila.dilute();
    ASSERT_NEAR(tequila.get_alcohol_content(),0.015,0.01);
