...
// TEST(ALCOHOL,test_alcohol_should_return_correct_name_price_content2){

//     Sour alcohol("item1",5,0.1);
//     ASSERT_EQ(alcohol.get_name(),"item1");
//     ASSERT_EQ(alcohol.get_price(),5);
//     ASSERT_EQ(alcohol.get_alcohol_content(),0.1);
...
TEST(ALCOHOL,test_alcohol_is_default_should_throw_exception){
    ...
    ASSERT_THROW(alcohol.get_name(),std::string);
    ASSERT_THROW(alcohol.get_price(),std::string);
    ASSERT_THROW(alcohol.get_alcohol_content(),std::string);
}

//
TEST(ALCOHOL,test_alcohol_copy_constructor){
